...
public:
	QRDecoder();
	static void DecodeQRCode(Mat & image, QRCode * qrCode);

private:
	static const int numRSBlock[][4] = 
	{
	...
	const static int numErrorCollectionCode[][4]= 
	{
	...
	void GetBlocks(int *& words);
	bool isInFunctionPattern(int targetX, int targetY);
	bool getElement(int x, int y);

	int getLogicalSeed(int x, int y);
	int * getSeed(int version);
	int getSeed(int version, int patternNumber);

	void decodeFormatInformation(bool * formatInformation);
